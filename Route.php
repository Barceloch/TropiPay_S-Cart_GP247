...
use App\GP247\Plugins\TropiPay\Controllers\FrontController;
...
if(gp247_extension_check_active($config['configGroup'], $config['configKey'])) {
    ...
    Route::group(['prefix' => 'tropipay', 'middleware' => ['web']], function () {
        Route::get('process-order', [FrontController::class, 'processOrder'])->name('tropipay.process_order');
        Route::get('success', [FrontController::class, 'success'])->name('tropipay.success');
        Route::get('failed', [FrontController::class, 'failed'])->name('tropipay.failed');
        Route::post('webhook', [FrontController::class, 'webhook'])->name('tropipay.webhook');
    });
